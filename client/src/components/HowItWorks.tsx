    ...
    <section className="py-20 bg-muted/30" id="how-it-works">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden md:grid md:grid-cols-3 gap-8 relative">
          {/* Connecting Lines */}
          <div className="absolute top-16 left-1/6 right-1/6 h-0.5 bg-border hidden md:block" style={{ left: '20%', right: '20%' }} />
          
          {steps.map((step, index) => (
            <Card
              key={index}
              className="relative p-8 text-center hover-elevate"
              data-testid={`card-step-${index + 1}`}
            >
              <div className="flex flex-col items-center space-y-4">
                <div className="w-16 h-16 rounded-full bg-primary flex items-center justify-center text-primary-foreground relative z-10">
